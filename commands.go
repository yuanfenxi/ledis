...
import (
	...
	"github.com/yuanfenxi/ledis/internal"
)
...
type Cmdable interface {
	...
	LTTL(key string,values ...interface{}) *IntCmd
	LExpire(key string,values ...interface{}) *IntCmd
...
func (c *cmdable) LExpire(key string, values ...interface{}) *IntCmd {
	args := make([]interface{}, 2+len(values))
	args[0] = "lexpire"
	args[1] = key
	for i, value := range values {
...
func (c *cmdable) LTTL(key string, values ...interface{}) *IntCmd {
	args := make([]interface{}, 2+len(values))
	args[0] = "lttl"
	args[1] = key
	for i, value := range values {
